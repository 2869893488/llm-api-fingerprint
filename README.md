...
### 默认串行安全模式（防触发第三方风控封禁）与并发控制

> [!TIP]
> **为什么要默认关闭并发？**
> 很多第三方 API 平台（中转站、代理网关）部署了严格的反脚本/反自动化风控策略。若并发全开，短时间内向端点发送数百次并发请求（包含对抗后缀和特征探测）极易触发服务商的 WAF 防火墙，导致 **429 限流、API Key 被停用甚至账号被封禁**。
> 因此，**本工具默认采用全串行安全模式运行**（`concurrent: false` 且 `parallel_phases: false`），降低瞬时请求密度，最大程度避免触发上游风控。

"并行"包含两个可独立控制的维度：

1. **双 API 并行**（`concurrent: false` 默认关闭；传 `--concurrent` 开启）：开启时同一探测同时发给官方与未知 API；关闭时依次串行发送；
2. **阶段并行**（`parallel_phases: false` 默认关闭；传 `--parallel-phases` 开启）：开启时一至六阶段同时推进；关闭时按阶段逐一执行。

**限速保护（防触发模型并发限制）**：开启并发时瞬时请求率会放大约 10 倍，内置**全局限速器**（官方与未知共用、所有阶段共享）：

- `auto_ratelimit: true`（默认）：**自动限速**。按端点响应头 `x-ratelimit-*` 自动换算 RPM/TPM 并取保守值，遇 429 间隔自动加倍；
- `max_inflight: 4`（默认）：同时在飞请求数上限，杜绝并发尖峰；
- 手动固定限速：`auto_ratelimit: false` 后 `rate_limit_rpm` 填写固定限速值（如 60 或 120）。

时间账参考：

| 模式 | 特点 | 总时长 |
|------|------|--------|
| **默认全串行安全模式** | 最安全、防触发第三方商户严格风控/封号 | **≈11～13 分钟** |
| 串行模式 + 官方侧缓存生效 | 第二次运行只请求未知侧 | **≈5～7 分钟** |
| 双维度并发开启（`--concurrent --parallel-phases`） | 耗时最短，适用于官方 API 或无风控的自建端点 | **≈6～8 分钟** |
| 双维度并发 + 官方侧缓存生效 | 耗时极短 | **≈3～4 分钟** |
...
- **1.3.0**（当前）—— 默认模式调整为全串行安全模式（降低请求密集度，防触发第三方平台/网关严格风控与封号）、新增 `--concurrent` 并发开关、新增 Token 消耗不对称红旗检测（隐藏 system prompt 注入、附加输出/水印、推理型模型替换检测）
- **1.2.0** —— 六阶段流水线：单 Token 指纹（阶段六，含"content 空 → logprobs 补读 → max_tokens=2 重试"三级读取兜底）、严格格式遵循与拒答画像（阶段一）、证据加权 + CI 可靠性降权 + 证据冲突降级、官方自比/异源对照基线、官方侧响应缓存、阶段二 20 探测 × 3 采样 + 边界补测 + max_tokens=96 精简档（对照实验 `verify_phase2_config.py` 验证误判率无差异）、分区进度面板、双维度并行 + 全局限速（响应头/429 感知自动限速）、代理披露/失败率红旗、调研与自测脚本（`analyze_phase2.py` / `tests/run_mock_test.py` 9 场景）
- **1.1.0** —— 五阶段：采样自洽画像（阶段五）、身份红旗扩展、fail-closed 证据规则
- **1.0.0** —— 初版：四阶段 + 断点续跑 + 取证存储

## 免责声明

1. **研究与技术交流用途**：本工具仅供技术研究、学术交流及合法授权下的 API 模型行为特征与一致性验证使用。
2. **账号与封禁免责（重要）**：
   - 探测过程中涉及对抗后缀扰动、提示词探测、多轮采样等自动化请求。部分第三方 API 服务商（如中转站、代理网关）部署了严格的 WAF 规则与反自动化风控机制。
   - **因使用本工具向任何第三方 API 端点发送探测请求而导致的 API Key 被停用、账号被限制/封禁、账户额度被扣除/清空或由此产生的任何直接与间接损失，本项目作者及贡献者概不承担任何责任。**
3. **合规使用与风险自担**：
   - 使用者在使用本工具前，应自行了解并严格遵守目标 API 平台的服务条款（Terms of Service）。请在获得合法授权或自建/合规测试环境下使用，一切风险与法律责任由使用者自行承担。
